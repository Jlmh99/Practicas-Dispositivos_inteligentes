...
| P.UMB | telefono_app + wearable_app + smart_pwa / Umbral crítico | Sesión activa, `activityStatus` = "JUGANDO:..." | 1. Presionar el ícono de bug ("Forzar umbral (demo)") en el AppBar. 2. Observar teléfono, wearable y TV. | `sessionSeconds` salta a 1795 y sube; al cruzar 1800, aparece: banner en el teléfono (`AlertBanner`), notificación local (`NotificacionesService`), `sessionState.alertaActiva = true`, y banner rojo en la TV ("Tiempo de juego prolongado, toma un descanso"). | Confirmado con capturas reales: notificación del sistema + `AlertBanner` simultáneos en el teléfono con `sessionSeconds` en 30:02 (`docs/evidencias/capalerta_1.png`); banner rojo en la TV con `sessionSeconds` en 30:40 y sin desbordar la card de "Sesión en curso" (`capalerta_2.png`) — confirma en vivo el fix del desfase duplicado y el fix del layout de la alerta. También se capturó la alerta secundaria de concentración baja (`capalerta.png`, 27% con "JUGANDO:SOPA-LETRAS"), fuera del alcance original de este caso pero evidencia extra de que `_evaluarUmbrales()` cubre los 3 umbrales. | Aprobado |
| P.DISC | wearable_app ↔ telefono_app / BLE | Conexión BLE activa, sesión en curso | Apagar el Bluetooth del emulador/dispositivo del wearable a media sesión. | `BleClient._onCambioConexion` detecta `disconnected`, emite `EstadoDesconectado`, limpia `_controlChar`/`_gameSelectChar`/`_timeOverrideChar`, y reintenta con backoff (2/4/8/16/30 s) sin crash de la app. | Confirmado con capturas reales: badge "Error: No se encontró el wearable" con la última métrica conocida congelada y `activityStatus` en "PAUSA", sin crash (`docs/evidencias/error no se encontro wereable.png`); y el estado "Buscando wearable..." del reintento automático (`buscando wereable.png`). | Aprobado |
...
cubren el requisito completo del mínimo de 5.)*

Evidencia adicional citada directamente en la tabla de casos (P.UMB, P.DISC):
`capalerta.png`, `capalerta_1.png`, `capalerta_2.png`, `buscando wereable.png`,
`error no se encontro wereable.png`. Más `loginpwa.png` (pantalla de login de
la PWA con cuenta real) como respaldo del flujo de autenticación.
